...
- [x] **P2** GraphQL gateway alongside REST — _Strawberry GraphQL schema at `/graphql` (GraphiQL IDE included); queries: usageMetrics, latencyMetrics, agents, agent(id), alerts_
- [x] **P2** SSE / WebSocket stream for live metrics push (replace polling) — _`GET /api/v1/metrics/stream` SSE endpoint; `useMetricsStream` React hook with auto-reconnect; live badge on Dashboard replacing 30s polling_
...
- [x] **P2** Vitest coverage for components (snapshot + interaction) — _22 tests across MetricCard (7), AgentHealthTable (7), LatencyChart (3), useMetricsStream hook (5); jsdom + @testing-library/react_
...
- [x] **P2** Distributed tracing demo with OpenAI mock + Ollama — _`examples/tracing-demo/` with mock_openai_server.py (FastAPI + OTel), demo_agent.py (W3C traceparent propagation), and README with Ollama variant_
...
- [x] **P2** ADR (Architecture Decision Records) folder — _`docs/adr/` with 6 ADRs: FastAPI, Prometheus, React+Vite, hierarchical auth, SSE vs WebSocket, Helm_
- [x] **P2** Tutorial: instrumenting a custom AI agent for Vectaris — _`docs/guides/instrumenting-custom-agents.md` with register, OTel init, LLM call tracing, heartbeat loop, complete example_
